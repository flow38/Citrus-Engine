...
{
import melon.system.component.statemachine.*;
...
import org.osflash.signals.Signal;
	...
	public class StateMachine
	{
		...
		private var _outEvent:StateMachineSignal;
		
		private var parentState:State;
		
		private var _parentStates:Array;
		
		private var _path:Array;

        private var _transitionComplete : Signal;

		private var _transitionDenied : Signal;

		private var _logger : IMelonLog;
		/**
		 ...
		 *	
		 *	playerSM.transitionDenied.add(transitionDeniedFunction);
		 *	playerSM.transitionComplete.add(transitionCompleteFunction);
		 *	
		 ...
		 *	</pre>
		*/
		public function StateMachine(id : String)
		{
			_id = id;
			_states = new Dictionary();
			_transitionComplete = new Signal(StateMachineSignal);
			_transitionDenied = new Signal(StateMachineSignal);
		}
		
		public function dispose():void
		{
			_states = null;
			_transitionComplete.removeAll();
			_transitionDenied.removeAll();
		}
		...
		**/
		public function addState(stateName:String, stateData:Object=null):void
		{
			if(stateName in _states){
				_logger != null && _logger.error(this, "[StateMachine] " + _id + " Overriding existing state " + stateName);
			}
		...
		**/
		public function set initialState(stateName:String):void
		{
			if (_state == null && stateName in _states){
				...
				var _callbackEvent : StateMachineSignal = new StateMachineSignal();
				_callbackEvent.toState = stateName;
					
				if(_states[_state].root){
					_parentStates = _states[_state].parents
					for(var j:int = _states[_state].parents.length-1; j>=0; j--){
						if(_parentStates[j].enter){
							_callbackEvent.currentState = _parentStates[j].name
							_parentStates[j].enter.call(null,_callbackEvent)
						}
					}
				}
				...
				_outEvent = new StateMachineSignal();
				_outEvent.toState = stateName;
				_transitionComplete.dispatch(_outEvent);
			}else {
					throw(new Error("State " + stateName + " must exist in states dictionnary, initialState shoudl be call after have add all states to state machine !"));
			}
		}
		...
		**/
		public function changeState(stateTo:String):void
		{
			...
			if (!(stateTo in _states)){
				_logger != null && _logger.warning(this, "[StateMachine] " + _id + " Cannot make transition: State " + stateTo + " is not defined");
				return;
			}
			...
			{
				_logger != null && _logger.warning(this, "[StateMachine] " + _id + " Transition from " + _state + " to "+ stateTo +" denied");
				_outEvent = new StateMachineSignal();
				_outEvent.fromState = _state;
				_outEvent.toState = stateTo;
				_outEvent.allowedStates = _states[stateTo].from;
				_transitionDenied.dispatch(_outEvent);
				return;
			}
			...
			if(_path[0]>0)
			{
				var _exitCallbackEvent:StateMachineSignal = new StateMachineSignal();
			...
			if(_path[1]>0)
			{
				...
				{
					_parentStates = _states[stateTo].parents
					for(var k:int = _path[1]-2; k>=0; k--)
					{
						if(_parentStates[k] && _parentStates[k].enter){
							_enterCallbackEvent.currentState = _parentStates[k].name;
							_parentStates[k].enter.call(null,_enterCallbackEvent);
						}
					}
				}
				 if(_states[_state].enter){
			...
			_logger != null && _logger.info(this, "[StateMachine]" + _id + "State Changed to " + _state);
		...
		public function get transitionDenied():Signal
		{
			return _transitionDenied;
		}
